...
import 'package:ppidunia/features/feed/data/reposiotories/comment/comment.dart';
...
class CommentScreenModel with ChangeNotifier {
  ...
  CommentScreenModel({required this.cr, required this.fr, required this.pp});
  ...
  Future<void> post({required String feedId}) async {
    try {
      ...
      await cr.post(feedId: feedId, comment: commentC.text);
  ...
  Future<void> getFeedDetail({required String feedId}) async {
    pageKey = 1;
    hasMore = true;
