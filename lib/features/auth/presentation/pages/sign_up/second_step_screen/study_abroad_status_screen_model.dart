...
import 'package:ppidunia/features/auth/presentation/pages/sign_up/widgets/search_bottomsheet_widget.dart';
...
abstract class StudyAbroadStatusScreenModelData {
  ...
  Future<void> getCountries(BuildContext context,
      {required int page, required String query});
  Future<List<CountryData>> searchCountries(BuildContext context,
      {required int page, required String query});
}

class StudyAbroadStatusScreenModel
    with ChangeNotifier
    implements StudyAbroadStatusScreenModelData {
  final AuthRepo ar;
  ...
  void setselectedStudyAbroadOption(BuildContext context,
      StudyAbroadOptions value, StudyAbroadStatusScreenModel viewModel) {
  ...
  void _submissionValidation(BuildContext context, String selectedStudyAbroad) {
    if (selectedStudyAbroadOption == StudyAbroadOptions.empty) {
      ShowSnackbar.snackbar(context, getTranslated('STUDY_ABROAD_OPTION_0'), '',
          ColorResources.error);
      return;
    }
    if (selectedStudyAbroad == "-" &&
        selectedStudyAbroadOption == StudyAbroadOptions.abroad) {
      ShowSnackbar.snackbar(context, getTranslated('COUNTRY_NOT_SELECTED'), '',
          ColorResources.error);
      return;
    }
    if (selectedStudyAbroad != "-" &&
  ...
  void buildBottomSheet(
      BuildContext context, StudyAbroadStatusScreenModel viewModel) {
