...
class NewPasswordScreenModel
    with ChangeNotifier
    implements NewPasswordScreenModelData {
  final AuthRepo ar;
  ...
  bool submissionValidation(
    ...
    if (verificationCode.isEmpty) {
      ShowSnackbar.snackbar(context, getTranslated('VERIFICATION_CODE_EMPTY'),
          '', ColorResources.error);
      verificationCodeFn.requestFocus();
      return false;
    } else if (newPassword.isEmpty) {
      ShowSnackbar.snackbar(context, getTranslated('NEW_PASSWORD_EMPTY'), '',
          ColorResources.error);
      newPasswordFn.requestFocus();
      return false;
    } else if (newPassword.length < 8) {
      ShowSnackbar.snackbar(context, getTranslated('NEW_PASSWORD_LENGTH'), '',
          ColorResources.error);
      newPasswordFn.requestFocus();
      return false;
    } else if (confirmPassword.isEmpty) {
      ShowSnackbar.snackbar(context, getTranslated('OLD_PASSWORD_EMPTY'), '',
          ColorResources.error);
      confirmPasswordFn.requestFocus();
      return false;
    } else if (confirmPassword.length < 8) {
      ShowSnackbar.snackbar(context, getTranslated('CONFIRM_PASSWORD_LENGTH'),
          '', ColorResources.error);
      confirmPasswordFn.requestFocus();
      return false;
  ...
  void submitControllerValue(BuildContext context) {
    ...
    if (isClear) {
      setNewPassword(context,
          newPassword: confirmPassword, oldPassword: verificationCode);
    }
  ...
  Future<void> setNewPassword(
    ...
    try {
      ...
      ShowSnackbar.snackbar(context, getTranslated('PASSWORD_UPDATED'), '',
      ...
      if (e.toString().toLowerCase().contains('password') ||
          e.toString().toLowerCase().contains('old')) {
        ShowSnackbar.snackbar(
            context,
            getTranslated('INVALID_VERIFICATION_CODE'),
            '',
            ColorResources.error);
        verificationCodeFn.requestFocus();
      }
