...
abstract class ForgetPasswordScreenModelData {
  ...
  Future<void> forgetPassword(BuildContext context, {required String email,});
}

class ForgetPasswordScreenModel with ChangeNotifier implements ForgetPasswordScreenModelData{
  final AuthRepo ar;
  ...
  Future<void> forgetPassword(BuildContext context, {required String email,}) async {
    ...
    try {
      await ar.forgetPassword(email: email,);
      SharedPrefs.writeForgetEmail(email);
      ShowSnackbar.snackbar(context, getTranslated('VERIFICATION_CODE_SENT'), '', ColorResources.success);
