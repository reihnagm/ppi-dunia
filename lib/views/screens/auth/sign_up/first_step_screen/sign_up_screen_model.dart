...
import 'package:ppidunia/views/screens/auth/sign_up/second_step_screen/study_abroad_status_state.dart';
...
class SignUpScreenModel with ChangeNotifier implements SignUpScreenModelData{
  ...
  bool submissionValidation(
    ...
    String confirmPassword,
      ...
      if(firstName.isEmpty) {
        ShowSnackbar.snackbar(context, getTranslated('FIRST_NAME_EMPTY'), '', ColorResources.error);
        firstNameFn.requestFocus();
        return false;
      }
      else if(lastName.isEmpty) {
        ShowSnackbar.snackbar(context, getTranslated('LAST_NAME_EMPTY'), '', ColorResources.error);
        ...
        ShowSnackbar.snackbar(context, getTranslated('INVALID_PASSWORD_INPUT'), '', ColorResources.error);
        passwordFn.requestFocus();
        return false;
      }
  ...
  void submitControllerValue(BuildContext context) {
    ...
    final isClear = submissionValidation(context, firstName, lastName, email, password, confirmPassword);
    if(isAgree && isClear) {
      register(context, email: email, password: password, firstName: firstName, lastName: lastName);
    }
    notifyListeners();
  }
  ...
  @override
  Future<void> register(BuildContext context, {required String email, required String password, required String firstName, required String lastName})  async {
    setStateRegisterStatus(RegisterStatus.loading);
    try {
      UserModel um = await ar.register(email: email, password: password, firstName: firstName, lastName: lastName);
